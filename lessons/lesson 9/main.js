...
let allUsersDiv = document.getElementsByClassName(`bed`)[0];
let bed = ['Main','Products','About us','Contacts'];
for (const user of bed) {
    let ul = document.createElement(`li`);
    ul.classList.add(`string`);
    ul.innerText = user;
    allUsersDiv.appendChild(ul);
}
// - Є масив
...
for (const user of coursesAndDurationArray) {
    ...
    head.innerText = user.title;
    let p = document.createElement('p');
    p.innerText = `${user.monthDuration}`;
    p.classList.add('xxx');
    userBlock.append(head, p);
...
for (const simpson of simpsons) {
    ...
    let head = document.createElement('h2');
    head.innerText = simpson.name;
    let p = document.createElement('p');
    p.innerText = `${simpson.surname},${simpson.age},${simpson.info}`;
    p.classList.add('xxx');
